...
/***************刘德祥修改***************/
		auto_taking_off_speed = AUTO_TAKE_OFF_KP *(Ano_Parame.set.auto_take_off_height + 10 - wcz_hei_fus.out);
...
/******************************/
	...
	alt_val_2.out  = LIMIT(alt_val_2.out,-150,150);		//限幅 -150~150
}
...
{
	u8 out_en;
	out_en = (flag.taking_off != 0) ? 1 : 0;		//if (flag.taking_off) != 0,(out_en) = 1;other (out_en) = 0
	
	flag.thr_mode = THR_AUTO;//THR_MANUAL;		//油门自动回中
	
	loc_ctrl_1.exp[Z] = fs.alt_ctrl_speed_set + alt_val_2.out;		//期望值
	...
	alt_val_1.out = LIMIT(alt_val_1.out,-err_i_comp,MAX_THR *10);		//限幅
