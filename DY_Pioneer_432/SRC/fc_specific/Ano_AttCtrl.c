...
#elif (MOTOR_ESC_TYPE == 1)		//无刷电机不带刹车
...
void Att_2level_Ctrl(float dT_s,s16 *CH_N)
{
	...
	if(ABS(exp_rol_tmp + att_2l_ct.exp_rol_adj) < 5)		//0~5
	{
		att_2l_ct.exp_rol_adj += 0.1f *exp_rol_tmp *dT_s;
		att_2l_ct.exp_rol_adj = LIMIT(att_2l_ct.exp_rol_adj,-1,1);		//限幅 -1~1
	}
	...
	/*摇杆量转换为YAW期望角速度*/
	att_1l_ct.set_yaw_speed = (s32)(0.0023f *my_deadzone(CH_N[CH_YAW],0,65) *max_yaw_speed);		//-200.1~200.1
	/*最大YAW角速度限幅*/
	set_yaw_av_tmp = LIMIT(att_1l_ct.set_yaw_speed ,-max_yaw_speed,max_yaw_speed);		//限幅 -200~200
...
//	{
//		att_2l_ct.exp_yaw += (-dy_yaw)*dT_s;		//2.0f
////		att_2l_ct.exp_yaw += set_yaw_av_tmp *dT_s;
////		att_2l_ct.exp_yaw += dy_yaw;
////		dy_yaw += 0.1f;
////		if(dy_yaw - 5.0f > 0.0001f)
////			dy_yaw = 5.0f;
//	}
//	else if(CH_N[AUX2]<200)		//-200~200
//	{
//		att_2l_ct.exp_yaw += set_yaw_av_tmp *dT_s;
////		if(dy_yaw - 0.1f > 0.0001f)
////			dy_yaw -= 0.1f;
////		else if(dy_yaw + 0.1f < 0.0001f)
////			dy_yaw += 0.1f;
////		else
////			dy_yaw = 0.0f;
////		att_2l_ct.exp_yaw += set_yaw_av_tmp *dT_s;
...
void Att_1level_Ctrl(float dT_s)
{
	...
	ctrl_parameter_change_task();
		...
		/*目标角速度限幅*/
		att_1l_ct.exp_angular_velocity[ROL] = LIMIT(att_1l_ct.exp_angular_velocity[ROL],-MAX_ROLLING_SPEED,MAX_ROLLING_SPEED);		//-1600~1600
