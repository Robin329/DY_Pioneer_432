...
void IMU_Update_Task(u8 dT_ms)		//姿态解算更新
{
...
void Mag_Update_Task(u8 dT_ms)
{
	...
	Mag_Get(mag_val);		//获取三轴地磁信息	;输出==>mag_val[i]
	
	Mag_Data_Deal_Task(dT_ms,mag_val,imu_data.z_vec[Z],sensor.Gyro_deg[X],sensor.Gyro_deg[Z]);		//磁力计校准	;输出==>mag_val[i]（校准之后）
