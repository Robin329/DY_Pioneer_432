...
void Power_UpdateTask(u8 dT_ms)
{
	...
	LPF_1_(cut_off_freq,dT_ms*1e-3f,voltage_s16,voltage_f);		//输出;==>voltage_f
	...
	if(Plane_Votage<Ano_Parame.set.lowest_power_voltage)		//最低电压
	{
		flag.power_state = 3;
		
	}

	if(Plane_Votage<Ano_Parame.set.warn_power_voltage)		//警告电压
	{
		if(LED_state>115)
		{
			LED_state = 1;
	...
	if(Plane_Votage<Ano_Parame.set.return_home_power_voltage)		//返航电压
	{
