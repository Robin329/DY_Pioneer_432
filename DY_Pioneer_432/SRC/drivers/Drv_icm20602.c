...
void mpu_auto_az()
{
	if(sensor.acc_z_auto_CALIBRATE)		//开机自动对准Z轴
	{
		acc_z_auto_cnt++;
...
///////////////////////////////////////////////////////////
		...
		{	
			...
			if(sensor.gyr_CALIBRATE)		//陀螺仪校准		sensor.gyr_CALIBRATE != 0
			{
				LED_state = 2;
				gyro_sum_cnt++;
...
//					printf("GYR init OK!\r\n");
			...
			if(sensor.acc_CALIBRATE == 1)		//加速度计校准
			{
				LED_state = 3;
				acc_sum_cnt++;
...
void Center_Pos_Set()
{
	center_pos.center_pos_cm[X] = Ano_Parame.set.center_pos_cm[X];//+0.0f;	重心相对传感器位置偏移量
...
//	sensor_rotate_func(dT);
	...
	/*静止检测*/
	motionless_check(dT_ms);		//==>flag.motionless
			
	MPU6050_Data_Offset();		//校准函数(加速度计和陀螺仪)
...
//	if(cnt++ % 2000 == 0)
//	{
//		printf("ACC_X:%d\r\n",sensor.Acc_Original[X]);
//		printf("ACC_Y:%d\r\n",sensor.Acc_Original[Y]);
//		printf("ACC_Z:%d\r\n",sensor.Acc_Original[Z]);
//		printf("GYRO_X:%d\r\n",sensor.Gyro_Original[X]);
//		printf("GYRO_Y:%d\r\n",sensor.Gyro_Original[Y]);
//		printf("GYRO_Z:%d\r\n",sensor.Gyro_Original[Z]);
//		printf("sensor.Tempreature_C:%.2f\r\n",sensor.Tempreature_C);
//	}
	...
	{ 
		...
		sensor_val[A_X+i] = sensor.Acc_Original[i] ;		//校准后的加速度计数据

		sensor_val[G_X+i] = sensor.Gyro_Original[i] - save.gyro_offset[i] ;		//校准后的陀螺仪数据
