...
void PWM_IN_Init ( void )
{
    ...
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = NVIC_PWMIN_P;		//抢占优先级1
    NVIC_InitStructure.NVIC_IRQChannelSubPriority = NVIC_PWMIN_S;		//响应优先级0
...
//////////////////////////////////////////////////////////////////////////////////////////////
	pwminmode = Ano_Parame.set.pwmInMode;
...
void _TIM3_IRQHandler ( void )
{
    ...
    if ( TIM3->SR & TIM_IT_CC1 )
    {
		...
		if(pwminmode == PWM)	//判断接收机模式，如果为PWM模式，则通道1正常接收pwm信号
		{
		...
		else if(pwminmode == PPM)	//判断接收机模式，如果为PPM模式，则使用通道1进行PPM信号解析
		{
			static u8 ch_sta = 0;
			if ( GPIOC->IDR & GPIO_Pin_6 )
			{
