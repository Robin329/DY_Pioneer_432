...
void AnoOF_GetOneByte(uint8_t data)
{
	...
	else if(state==5)		//最后一字节
	{
		state = 0;
		_datatemp[4+_data_cnt]=data;
		AnoOF_DataAnl(_datatemp,_data_cnt+5);//anoof_data_ok = 1 ;//_datatemp数据帧，_data_cnt+5帧长度
	}
	else
		state = 0;
}
...
void AnoOF_DataAnl(uint8_t *data_buf,uint8_t num)		//光流数据解析函数
{
	u8 sum = 0;
...
//			OF_ALT = (uint16_t)(*(data_buf+5)<<8)|*(data_buf+6) ;
			...
			OF_ALT = (uint16_t)(*(data_buf+6)<<8)|*(data_buf+5) ;
			of_check_f[1] = 1;
		}
