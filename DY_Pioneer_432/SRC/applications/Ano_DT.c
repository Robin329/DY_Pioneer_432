...
/////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////
	ANO_DT_Data_Receive_Anl_Task();		//数传数据解析
/////////////////////////////////////////////////////////////////////////////////////
	Usb_Hid_Send();					
/////////////////////////////////////////////////////////////////////////////////////
}
...
void ANO_DT_Send_Test_u8(u8 data1)
{
	u8 _cnt=0;
	data_to_send[_cnt++]=0xAA;		//0
	data_to_send[_cnt++]=MYHWADDR;		//1
	data_to_send[_cnt++]=SWJADDR;		//2
	data_to_send[_cnt++]=0x00;		//3
	data_to_send[_cnt++]=0;		//4
	
	data_to_send[_cnt++]=data1;
//	data_to_send[_cnt++]=BYTE1(hardware_ver);		//数据拆分
//	data_to_send[_cnt++]=BYTE0(hardware_ver);
//	data_to_send[_cnt++]=BYTE1(software_ver);
//	data_to_send[_cnt++]=BYTE0(software_ver);
//	data_to_send[_cnt++]=BYTE1(protocol_ver);
//	data_to_send[_cnt++]=BYTE0(protocol_ver);
//	data_to_send[_cnt++]=BYTE1(bootloader_ver);
//	data_to_send[_cnt++]=BYTE0(bootloader_ver);		//13	_cnt=14
	
	data_to_send[4] = _cnt-5;		//9
	
	u8 sum = 0;
	for(u8 i=0;i<_cnt;i++)
		sum += data_to_send[i];
	data_to_send[_cnt++]=sum;		//14	校验位
	
	ANO_DT_Send_Data(data_to_send, _cnt);
}

void ANO_DT_Send_Test_u32(u32 data1)
{
	u8 _cnt=0;
	data_to_send[_cnt++]=0xAA;		//0
	data_to_send[_cnt++]=MYHWADDR;		//1
	...
	data_to_send[_cnt++]=BYTE3(data1);		//数据拆分
